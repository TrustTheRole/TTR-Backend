    ...
    B::Data: Send,
{
    let key_path = match env::var("KEY_PATH") {
        Ok(path) => path,
        Err(e) => {
            tracing::debug!("KEY_PATH env variable error: {}", e);
            return Err((
    ...
    let private_key_pem = match fs::read_to_string(&key_path) {
        ...
        Err(e) => {
            tracing::debug!("Error reading private key file: {}", e);
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({
                    "error": e.to_string()
    ...
    let private_key = match RsaPrivateKey::from_pkcs1_pem(&private_key_pem) {
        ...
        Err(e) => {
            tracing::debug!("Error parsing private key: {}", e);
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({
                    "error": e.to_string()
    ...
    let json: Value = match serde_json::from_slice(&bytes) {
        ...
        Err(e) => {
            tracing::debug!("Error parsing JSON from body: {}", e);
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({
                    "error": e.to_string()
    ...
    let data = match json.get("encrypted_data") {
        Some(data) => data,
        None => {
            tracing::debug!("Missing encrypted_data field in JSON body");
            return Err((
    ...
    let cipher_text = match general_purpose::STANDARD.decode(&encrypted_data) {
        Ok(c_text) => c_text,
        Err(e) => {
            tracing::debug!("Error decoding base64 encrypted data: {}", e);
            return Err((
                StatusCode::EXPECTATION_FAILED,
                Json(json!({
                    "error": e.to_string()
    ...
    let decrypted_data = match private_key.decrypt(padding, &cipher_text) {
        Ok(d_data) => d_data,
        Err(e) => {
            tracing::debug!("Decryption error: {}", e);
            return Err((
                StatusCode::EXPECTATION_FAILED,
                Json(json!({
                    "error": e.to_string()
    ...
    let decrypted_string = match String::from_utf8(decrypted_data) {
        Ok(d_data) => d_data,
        Err(e) => {
            tracing::debug!("Error converting decrypted data to string: {}", e);
            return Err((
                StatusCode::EXPECTATION_FAILED,
                Json(json!({
                    "error": e.to_string()
    ...
    let decrypted_json: Value = match serde_json::from_str(&decrypted_string) {
        Ok(json_data) => json_data,
        Err(e) => {
            tracing::debug!("Failed to parse decrypted data as JSON: {}", e);
            return Err((
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(json!({
    ...
    tracing::debug!("Decrypted JSON: {}", decrypted_json);
